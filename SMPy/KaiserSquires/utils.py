...
def fft2_shifted(data):
    """
    Compute the 2D Fourier Transform with a shift to center the zero frequencies.
    """
    return np.fft.fftshift(np.fft.fft2(data))

def ifft2_shifted(data):
    """
    Compute the inverse 2D Fourier Transform with a shift to center the zero frequencies.
    """
...
def calculate_field_boundaries(shear_df, resolution, width):
    """
    Calculate the boundaries of the field in right ascension (RA) and declination (Dec).

    :param shear_df: Pandas DataFrame containing the shear data with 'ra' and 'dec' columns.
...
def create_shear_grid(ra, dec, g1, g2, weight, boundaries, npix):
    ...
    boundaries : array_like
        Boundaries of the field of view, given as (xmin, xmax, ymin, ymax).
    npix : int or (int, int)
    ...
    weight_grid[weight_grid != 0] = 1 / weight_grid[weight_grid != 0]
    g1_grid *= weight_grid
    g2_grid *= weight_grid
